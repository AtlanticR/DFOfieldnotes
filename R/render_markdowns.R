#' the render_markdowns function
#'
#' This function allows you to render the parameterized reports
#'
#' @param ids defaults to projectids as defined in run_reports function
#'
#' @keywords render
#' @export output.pdf
#' @examples
...
render_markdowns <- function(ids = projectids) {
  ...
  print(ids)
  # index file
  index_file <- file.path(Rmdpath, "index.Rmd")
  ...
  markdowns <- lapply(
    ...
    function(x, filename){
      ...
      rmarkdown::render(filename, params = list(id = x, idnum=which(x==ids)), run_pandoc = FALSE)
  ...
  #rmarkdown::draft(to, template="jss", package="rticles")
